...
exports.UrlSafetyService = void 0;
exports.isPrivateAddress = isPrivateAddress;
exports.runBasicUrlChecks = runBasicUrlChecks;
const promises_1 = require("node:dns/promises");
const node_net_1 = __importDefault(require("node:net"));
const ai_link_safety_service_1 = require("./ai-link-safety-service");
...
function isPrivateAddress(address) {
    ...
    if (node_net_1.default.isIP(normalized) === 4)
        return isPrivateIpv4(normalized);
    if (node_net_1.default.isIP(normalized) !== 6)
        return true;
...
/** Exported for focused, network-free tests. */
function runBasicUrlChecks(rawUrl) {
...
async function assertPublicDestination(url) {
    ...
    if (node_net_1.default.isIP(hostname)) {
        if (isPrivateAddress(hostname))
            throw new UnsafeDestinationError("Private network destination is blocked");
        return;
    }
...
async function fetchPageSafely(originalUrl) {
    ...
    for (let redirectCount = 0; redirectCount <= MAX_REDIRECTS; redirectCount += 1) {
        ...
        if (response.status >= 300 && response.status < 400) {
            ...
            if (!location)
                throw new Error("Destination returned an invalid redirect");
...
function applyPageChecks(page, score, signals) {
...
function getRiskLevel(status, score) {
    if (status === "unsafe" || score >= 75)
        return "critical";
    if (score >= 45)
        return "high";
    if (score >= 20 || status === "suspicious")
        return "medium";
    return "low";
}
exports.UrlSafetyService = {
    async scanUrl(originalUrl) {
        ...
        const aiSafetyResult = await ai_link_safety_service_1.AiLinkSafetyService.analyze({
        ...
        const aiSafetyAnalysis = aiSafetyResult.analysis ?? {
            status: "unavailable",
            verdict: null,
            confidence: 1,
            reasons: [],
        ...
        if (aiSafetyAnalysis.status === "available") {
            if (aiSafetyAnalysis.verdict === "unsafe")
                score = Math.max(score + 20, 75);
            if (aiSafetyAnalysis.verdict === "suspicious")
                score += 15;
        }
        score = clamp(score, 0, 100);
        const safetyStatus = score >= 75 ? "unsafe" : score >= 35 ? "suspicious" : "safe";
        return {
            safetyStatus,
